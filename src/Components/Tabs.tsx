  ...
  const renderContent = () => {
    switch (activeTab) {
      case "Office Furniture":
        return <Slikslider category="Living Room" />;
      case "Outdoor Furniture":
        return <Slikslider category="Outdoor" />;
      case "Bedroom Furniture":
        return <Slikslider category="Bedroom" />;
      default:
        return null;
    }
  };
  ...
  return (
    <div className="w-full bg-[#FFFFFF99] rounded-[30px] border-[0.7px] mt-[40px] p-5 pt-20 border-white font-inter">
      ...
      <div className="flex rounded-t-[20px] overflow-hidden justify-center">
        {["Office Furniture", "Outdoor Furniture", "Bedroom Furniture"].map(
          (tab, index) => (
            <button
              key={tab}

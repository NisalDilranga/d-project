...
import { NavbarEvents } from "../Components/EcommerceNavbar";
...
const ProductDetails = () => {
  ...
  const handleAddToCart = async () => {
    ...
    if (!selectedWoodType && product.woodTypes?.length > 0) {
      message.warning("Please select a wood type");
      return;
    }
    ...
    try {
      // Get price with wood type multiplier
      const price = selectedWoodType
        ? product.basePrice * selectedWoodType.priceMultiplier
        : product.basePrice;

      // Call API to add to cart
      const woodTypeId = selectedWoodType ? selectedWoodType._id : null;
      await addToCart(product._id, quantity, woodTypeId);
